...
func (w *Writer) Write(pkg Package) error {
	w.pkg = pkg.Package

	w.init()
	w.line(`// Code generated by gtrace. DO NOT EDIT.`)

	for i, line := range pkg.BuildConstraints {
		if i == 0 {
			w.line()
		}
		w.line(line)
	...
	w.line()
	w.line(`package `, pkg.Name())
	w.line()

	var deps []dep
	for _, trace := range pkg.Traces {
...
func ident(str string) string {
	...
	str = strings.Map(func(rn rune) rune {
		switch {
		case rn == '_' ||
			unicode.IsLetter(rn) ||
			unicode.IsNumber(rn):
